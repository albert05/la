...
<div id="page-content" class="profile2">
	<div class="bg-success clearfix">
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<h4 class="name">{{ $workconfig->$view_col }}</h4>
					<div class="row stats">
						<div class="col-md-6 stat"><div class="label2" data-toggle="tooltip" data-placement="top" title="Designation">{{ $workconfig->designation }}</div></div>
						<div class="col-md-6 stat"><i class="fa fa-map-marker"></i> {{ $workconfig->city or "NA" }}</div>
					</div>
					<p class="desc">{{ substr($workconfig->about, 0, 33) }}@if(strlen($workconfig->about) > 33)...@endif</p>
				</div>
			</div>
		</div>
		<div class="col-md-3">
			<div class="dats1"><i class="fa fa-envelope-o"></i> {{ $workconfig->email }}</div>
			<div class="dats1"><i class="fa fa-phone"></i> {{ $workconfig->mobile }}</div>
			<div class="dats1"><i class="fa fa-clock-o"></i> Joined on {{ date("M d, Y", strtotime($workconfig->date_hire)) }}</div>
		</div>
		<div class="col-md-4">
			<div class="teamview">
		...
		<div class="col-md-1 actions">
			@la_access("WorkConfigs", "edit")
				<a href="{{ url(config('laraadmin.adminRoute') . '/workconfigs/'.$workconfig->id.'/edit') }}" class="btn btn-xs btn-edit btn-default"><i class="fa fa-pencil"></i></a><br>
			@endla_access
			
			@la_access("WorkConfigs", "delete")
				{{ Form::open(['route' => [config('laraadmin.adminRoute') . '.workconfigs.destroy', $workconfig->id], 'method' => 'delete', 'style'=>'display:inline']) }}
	...
	<ul data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		<li class=""><a href="{{ url(config('laraadmin.adminRoute') . '/workconfigs') }}" data-toggle="tooltip" data-placement="right" title="Back to WorkConfigs"><i class="fa fa-chevron-left"></i></a></li>
		...
		@if($workconfig->id == Auth::user()->id || Entrust::hasRole("SUPER_ADMIN"))
